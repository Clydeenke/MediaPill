...
> **学到的东西**：`.gitignore` 是"我需要 git 忽略的文件列表"。不配置的话，build 过程中的临时文件也会被推到仓库——GitHub 项目充斥几百个 .class 文件和几个 G 的 .gradle 缓存，别人 clone 下来要几分钟。`*.keystore` 不进 git 是因为私钥泄露 = 整个模块签名被别人伪造。
